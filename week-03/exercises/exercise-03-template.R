	...
	# One site observed in one year
	# 3. Which column names appear to contain more than one variable?
	# the codj, coda, hadj, hada, polj, and pola contain species and juvenile/adult.
	...
	# Answer in a comment:
	#
	# What does bind_rows() do? Bind_rows combines the two tables to make one wider table with both lower and upper bay observations.
	...
	# the region, site, and year
	# 2. What does one row represent after the pivot?
	# the region, site, year, and species/life stage specification
	# 3. Why does species_stage still violate the rules of tidy data?
	# codj and coda and other titles are not descriptive and contain two variables (species and life stage)
	...
	#The observations in one year at one site in the upper or lower bay of a species in a specific life stage
	...
	# Pollock, specifically juveniles
