...
# Date: 09/15/2026
  ...
  total_mass <- sum(fish_mass , na.rm = TRUE)
	...
	# 4. Display all observations from fish at least 35 cm long.
	  ...
	  greater_than_35_cm <- survey_data[survey_data$length_cm > 35, ]
	  greater_than_35_cm
	
	# 5. Display only the observations where a fish was caught.
	  fish_caught <- survey_data[survey_data$caught == TRUE, ]
	  fish_caught
	...
	# of survey_data.
	  ...
	  large_and_caught <- survey_data[survey_data$length_cm >= 35 & survey_data$caught == TRUE, ]
	  large_and_caught
	...
	#
	# Response: I learned how to use the length function and the '&' symbol. 
